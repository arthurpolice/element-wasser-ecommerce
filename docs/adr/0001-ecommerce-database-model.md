...
Use Better Auth `User` only for authentication and link it optionally to a commerce `Customer`; guest checkout creates a separate customer per order. Registered customer name and email stay synced with the linked user, while orders store customer contact and address snapshots.
...
Keep carts outside the database for now. Create an order only when checkout is placed, reserve stock for 15 minutes while payment is pending, and allow multiple Stripe or TWINT payment attempts through a separate `Payment` table.

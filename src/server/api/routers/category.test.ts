    ...
    category: {
      findMany: vi.fn(
        async ({
          ...
          include?: { _count?: { select: { products: boolean; children: boolean } } };
        }) => {
          let rows = categories;

          if (where?.parentId !== undefined) {
            rows = rows.filter((category) => category.parentId === where.parentId);
          }
          ...
          if (include) {
            return rows.map((row) => ({
              ...
              _count: {
                ...
                children: categories.filter((child) => child.parentId === row.id)
                  .length,
              },
            }));
          }
          ...
          if (include?._count) {
            return {
              ...
              _count: {
                ...
                children: categories.filter((child) => child.parentId === match.id)
                  .length,
              },
            };
          }
...
describe("category router move", () => {
  ...
  it("moves a category after a sibling at root level", async () => {
    ...
    expect(categories.find((category) => category.id === "child-a1")?.parentId).toBe(
      null,
    );
  });
